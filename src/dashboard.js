...
function recordsCard(root) {
  ...
  root.appendChild(el('div', 'section-label', 'Records · all time'));
...
function vitalsRow(chId, e, color) {
  ...
  keys.forEach(k => {
    ...
    RENDER.push(() => { sp.replaceChildren(); const W = Math.max(sp.clientWidth || 120, 60), H = 26; sp.setAttribute('viewBox', `0 0 ${W} ${H}`); const nn = vals.filter(x => x != null); if (nn.length < 2) return; const lo = Math.min(...nn), hi = Math.max(...nn), pad = (hi - lo) * .15 || 1; const pts = vals.map((x, j) => x == null ? null : [2 + j / 89 * (W - 4), H - 3 - (x - lo + pad) / (hi - lo + 2 * pad) * (H - 6)]); runs(pts, 3).forEach(r2 => r2.length > 1 && sv('path', { d: poly(r2), fill: 'none', stroke: css(def.color || color), 'stroke-width': 1.8, 'stroke-linecap': 'round', 'stroke-linejoin': 'round' }, sp)); const last = [...pts].reverse().find(Boolean); if (last) sv('circle', { cx: last[0], cy: last[1], r: 2.6, fill: css(def.color || color) }, sp); });
    row.appendChild(c);
  });
...
function render() {
  ...
  requestAnimationFrame(() => { runRender(); ZOOM = null; });
