...
const SHELL = () => {
  ...
  if (document.documentElement.scrollWidth > vw + 1) issues.push(`page scrolls sideways: ${document.documentElement.scrollWidth} > ${vw}`);
  document.querySelectorAll('h1, h2, .lede, .lp-lede, .lp-tile .th span, .lp-steps b, .lp-notes li, .lp-nav a, .facts b, .facts span, .steps li, .primary, .ghost, .danger, .hint, .fields label, .brandmark b, .pstat, .pfile, .lnk').forEach(e => {
    const r = e.getBoundingClientRect(); if (!r.width) return;
